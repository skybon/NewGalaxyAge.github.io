...
def update_fit(eft_filename, rst_filename):
    ...
    with open(rst_filename, 'w') as f:
        ...
        starcount = os.path.basename(os.path.dirname(rst_filename))
        if isNumber(starcount):
            starcount = int(starcount)
        else:
            try:
                starcount = starList[starcount]
            except:
                logging.error("Фит без звезд - %s", rst_filename)

        if 0 < starcount < 6:
            minuscount = 5 - starcount
            fit_name = starcount * '*' + minuscount * '-' + ' ' + fit_name
            f.write('.. index:: %s\n\n' % fit_name)
